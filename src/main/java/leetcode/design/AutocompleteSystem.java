 ...
 * historical data. Sentences is a leetcode.string leetcode.array consists of previously typed sentences. Times is the
...
public class AutocompleteSystem {
   ...
   *
   * @param sentences leetcode.array of sentences
   * @param times degree
   */
    ...
    if (c == '#') {
      ...
      if (curr != null) {
        if (curr.containsChild(c)) {
          ...
          // the hot-text for the current and child nodes hence we have to attach the prefix leetcode.string
  ...
  private class Trie {
    ...
    private void add(int i, String word, int length) {
      ...
      } else map.put(null, new Trie()); // use null to indicate end of leetcode.string
    }
