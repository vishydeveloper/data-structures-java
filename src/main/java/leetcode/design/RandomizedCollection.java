 ...
 * indices are indices of leetcode.array containing the value. Insert: Insert a element in end of leetcode.array and
 * add the index of leetcode.array as the set of values in hashmap. Remove: If the hashmap contains value
 * remove a random element from the set and replace the element at that index with the last element
 * from leetcode.array and remove the last element from the leetcode.array. Since we are removing the last element
 * from leetcode.array this operation requires only O(1) time getRandom(): Generate a random number between 0
 * and size of leetcode.array and return the element at that position.
 */
