...
/**
 * Created by gouthamvidyapradhan on 30/04/2019 A full binary leetcode.tree is a binary leetcode.tree where each node
 * has exactly 0 or 2 children.
 *
 * <p>Return a list of all possible full binary trees with N nodes. Each element of the answer is
 * the root node of one possible leetcode.tree.
 *
 * <p>Each node of each leetcode.tree in the answer must have node.val = 0.
 *
 ...
 * <p>1 <= N <= 20
 *
 * <p>Solution: O(2 ^ N) A full binary leetcode.tree can only be generated for a case with odd number of
 * nodes. Start with a base case - when only one node is given a full binary leetcode.tree can be formed with
