...
/**
 * Created by gouthamvidyapradhan on 30/07/2019 You need to construct a binary leetcode.tree from a leetcode.string
 * consisting of parenthesis and integers.
 *
 * <p>The whole input represents a binary leetcode.tree. It contains an integer followed by zero, one or two
 * pairs of parenthesis. The integer represents the root's value and a pair of parenthesis contains
 * a child binary leetcode.tree with the same structure.
 *
 * <p>You always start to construct the left child node of the parent first if it exists.
 *
 * <p>Example: Input: "4(2(3)(1))(6(5))" Output: return the leetcode.tree root node representing the
 * following leetcode.tree:
 *
 * <p>4 / \ 2 6 /\ / 3 1 5
 *
 * <p>Note: There will only be '(', ')', '-' and '0' ~ '9' in the input leetcode.string. An empty leetcode.tree is
 * represented by "" instead of "()".
 *
 * <p>Solution: O(N ^ 2) Form a node for every number and treat the first sub-leetcode.string within round
 * braces as left node and the second sub-leetcode.string within the round braces as the right node.
