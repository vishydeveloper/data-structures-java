...
/**
 * Created by gouthamvidyapradhan on 06/08/2019 For a binary leetcode.tree T, we can define a flip operation
 * as follows: choose any node, and swap the left and right child subtrees.
 *
 * <p>A binary leetcode.tree X is flip equivalent to a binary leetcode.tree Y if and only if we can make X equal to Y
 ...
 * <p>Note:
 *
 * <p>Each leetcode.tree will have at most 100 nodes. Each value in each leetcode.tree will be a unique integer in the
 * range [0, 99]. Solution O(N ^ 2) Since the node values are unique general idea is to find the
 * node on right leetcode.tree for every node on the left leetcode.tree and check if the values need to be swapped, if
 * yes then swap the node's left and right child in the left leetcode.tree. After this operation is complete
