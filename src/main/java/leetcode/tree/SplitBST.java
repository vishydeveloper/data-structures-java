 ...
 * root, and a target value V, split the leetcode.tree into two subtrees where one subtree has nodes that are
 * all smaller or equal to the target value, while the other subtree has all nodes that are greater
 * than the target value. It's not necessarily the case that the leetcode.tree contains a node with value V.
 *
 * <p>Additionally, most of the structure of the original leetcode.tree should remain. Formally, for any
 * child C with parent P in the original leetcode.tree, if they are both in the same subtree after the split,
 ...
 *
 * <p>The given leetcode.tree [4,2,6,1,3,5,7] is represented by the following diagram:
 *
 * <p>4 / \ 2 6 / \ / \ 1 3 5 7
 *
 ...
 * the left sub-leetcode.tree. Split the right node further recursively
 */
