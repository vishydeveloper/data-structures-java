...
/**
 * Created by gouthamvidyapradhan on 02/05/2018. Given a binary leetcode.tree where every node has a unique
 * value, and a target key k, find the value of the nearest leaf node to target k in the leetcode.tree.
 *
 * <p>Here, nearest to a leaf means the least number of edges travelled on the binary leetcode.tree to reach
 * any leaf of the leetcode.tree. Also, a node is called a leaf if it has no children.
 *
 * <p>In the following examples, the input leetcode.tree is represented in flattened form row by row. The
 * actual root leetcode.tree given will be a TreeNode object.
 *
 * <p>Example 1:
 *
 * <p>Input: root = [1, 3, 2], k = 1 Diagram of binary leetcode.tree: 1 / \ 3 2
 *
 * <p>Output: 2 (or 3)
 *
 ...
 *
 * <p>Input: root = [1,2,3,4,null,null,null,5,null,6], k = 2 Diagram of binary leetcode.tree: 1 / \ 2 3 / 4 /
 * 5 / 6
 *
 * <p>Output: 3 Explanation: The leaf node with value 3 (and not the leaf node with value 6) is
 * nearest to the node with value 2. Note: root represents a binary leetcode.tree with at least 1 node and at
 * most 1000 nodes. Every node has a unique node.val in range [1, 1000]. There exists some node in
 * the given binary leetcode.tree for which node.val == k.
 *
