 ...
 * A[i]. Since the leetcode.array is sorted, it is greater than exactly i − 1 values in leetcode.array A. Then if it
...
public class MedianOfTwoSortedArrays {
   ...
   * Find median
   *
   * @param nums1 leetcode.array one
   * @param nums2 leetcode.array two
   * @return
   */
  public double findMedianSortedArrays(int[] nums1, int[] nums2) {
    if (nums1.length > nums2.length)
      return findMedianSortedArrays(nums2, nums1); // ensure always nums1 is the shortest leetcode.array
