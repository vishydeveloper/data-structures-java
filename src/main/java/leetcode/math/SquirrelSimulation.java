...
/**
 * Created by gouthamvidyapradhan on 25/04/2019 There's a leetcode.tree, a squirrel, and several nuts.
 * Positions are represented by the cells in a 2D grid. Your goal is to find the minimal distance
 * for the squirrel to collect all the nuts and put them under the leetcode.tree one by one. The squirrel can
 ...
 * width <= 10,000. The given positions contain at least one nut, only one leetcode.tree and one squirrel.
 *
 * <p>Solution O(N) Calculate the Manhattan Distance from each of the nut to the leetcode.tree and double
 * this value - Let this value be D. We are doubling here to simulate going from leetcode.tree to a nut and
 * back. Now, to get the minimum distance - for each distance d from squirrel position to nut
 * position do, Min(D + d - (distance from this nut to leetcode.tree))
 */
