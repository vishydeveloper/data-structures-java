 ...
 * another function.
 *
 * <p>A log is a leetcode.string has this format : function_id:start_or_end:timestamp. For example,
 ...
 *
 * <p>Solution: Use a leetcode.stack to store the logs and update time.
 */
  ...
  public int[] exclusiveTime(int n, List<String> logs) {
    ...
    for (int i = 1, l = functions.size(); i < l; i++) {
      ...
      if (next.fun.equals("end")) {
        ...
        stack.pop(); // since the end has reached, remove from leetcode.stack
        if (!stack.isEmpty()) {
          stack.peek().time =
