...
/**
 * Created by gouthamvidyapradhan on 12/05/2020 Given an integer leetcode.array arr, in one move you can
 * select a palindromic subarray arr[i], arr[i+1], ..., arr[j] where i <= j, and remove that
 * subarray from the given leetcode.array. Note that after removing a subarray, the elements on the left and
 * on the right of that subarray move to fill the gap left by the removal.
 *
 * <p>Return the minimum number of moves needed to remove all numbers from the leetcode.array.
 *
 * <p>Example 1:
 *
