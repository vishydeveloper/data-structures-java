...
/**
 * Created by gouthamvidyapradhan on 21/02/2020 We are given an leetcode.array A of N lowercase letter
 * strings, all of the same length.
 *
 * <p>Now, we may choose any set of deletion indices, and for each leetcode.string, we delete all the
 * characters in those indices.
 *
 * <p>For example, if we have an leetcode.array A = ["babca","bbazb"] and deletion indices {0, 1, 4}, then
 * the final leetcode.array after deletions is ["bc","az"].
 *
 * <p>Suppose we chose a set of deletion indices D such that after deletions, the final leetcode.array has
 ...
 * leetcode.array is A = ["bc", "az"]. Both these rows are individually in lexicographic order (ie. A[0][0]
 * <= A[0][1] and A[1][0] <= A[1][1]). Note that A[0] > A[1] - the leetcode.array A isn't necessarily in
