...
/**
 * Created by gouthamvidyapradhan on 17/03/2019 You are given an integer leetcode.array A. From some starting
 ...
 * index, you can reach the end of the leetcode.array (index A.length - 1) by jumping some number of times
 ...
 *
 * <p>Solution: O(N log N) For each leetcode.array index and for each odd/even turn pre-calculate the next
 * jump index - this can be achieved in O(n log n) by using a balanced leetcode.tree. Check for each leetcode.array
 * index if we can reach end of the leetcode.array by using the pre-calculated values for next jump - cache
