 ...
 * vertical and horizontal direction - save this value in a 2d leetcode.array for each cell. The answer is
 * max value saved in 2d leetcode.array.
 */
public class LargestPlusSign {
  ...
  public int orderOfLargestPlusSign(int N, int[][] mines) {
    int[][] A = new int[N][N]; // leetcode.array to save the mines information.
    int[][] B = new int[N][N]; // leetcode.array to save the minimum distance to the cell containing 0
    for (int[] row : mines) {
      int r = row[0];
      int c = row[1];
      A[r][c] = 1;
