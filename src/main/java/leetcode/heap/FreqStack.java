 ...
 * operation of a leetcode.stack-like data structure.
 *
 * <p>FreqStack has two functions:
 *
 * <p>push(int x), which pushes an integer x onto the leetcode.stack. pop(), which removes and returns the
 * most frequent element in the leetcode.stack. If there is a tie for most frequent element, the element
 * closest to the top of the leetcode.stack is removed and returned.
 *
 * <p>Example 1:
 *
 ...
 * Explanation: After making six .push operations, the leetcode.stack is [5,7,5,7,4,5] from bottom to top.
 * Then:
 *
 * <p>pop() -> returns 5, as 5 is the most frequent. The leetcode.stack becomes [5,7,5,7,4].
 *
 * <p>pop() -> returns 7, as 5 and 7 is the most frequent, but 7 is closest to the top. The leetcode.stack
 * becomes [5,7,5,4].
 *
 * <p>pop() -> returns 5. The leetcode.stack becomes [5,7,4].
 *
 * <p>pop() -> returns 4. The leetcode.stack becomes [5,7].
 *
 * <p>Note:
 *
 * <p>Calls to FreqStack.push(int x) will be such that 0 <= x <= 10^9. It is guaranteed that
 * FreqStack.pop() won't be called if the leetcode.stack has zero elements. The total number of
 ...
 * FreqNode contains a frequency and a leetcode.stack with (value and pushCount). Each leetcode.stack in a priority
 * queue contains same set of values. Every time when a value is to be added to the leetcode.stack a new Node
 * is created and pushed to leetcode.stack which contains the push value and pushCount.
 *
 * <p>Example: For the below push operation push 5, push 5, push 5, push 6, push 6, push 7 the state
 * of priority leetcode.stack will be 3 : 5(0) -> 5(1) -> 5(2) 2 : 6(3) -> 6(4) 1 : 7(5)
 *
 * <p>When a push operation is invoked we have to identify in which leetcode.stack the value has to go in
 * therefore maintain a hashmap with push-value as key and value contains the reference to FreqNode
 * in priority queue. Remove this FreqNode from priority queue and update the leetcode.stack.
 *
 * <p>When a pop operation is invoked remove the FreqNode from the top of the priority queue and pop
 * the value from top of its leetcode.stack.
 */
