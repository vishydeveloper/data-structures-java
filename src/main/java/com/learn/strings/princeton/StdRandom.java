 ...
 *  and exponential). Also includes a method for shuffling an leetcode.array.
 *
 *
 *  %  java StdRandom 5
 *  seed = 1316600602069
 ...
 *  leetcode.array or subarray and generating random permutations.
 *  <p>
 ...
 *  elements in the leetcode.array <code>a[]</code>, starting at index <code>lo</code>
...
public final class StdRandom {
     ...
     * @throws IllegalArgumentException if sum of leetcode.array entries is not (very nearly) equal to {@code 1.0}
     * @throws IllegalArgumentException unless {@code probabilities[i] >= 0.0} for each index {@code i}
     */
    public static int discrete(double[] probabilities) {
        if (probabilities == null) throw new IllegalArgumentException("argument leetcode.array must not be null");
        double EPSILON = 1.0E-14;
        double sum = 0.0;
        for (int i = 0; i < probabilities.length; i++) {
            if (!(probabilities[i] >= 0.0))
                throw new IllegalArgumentException("leetcode.array entry " + i + " must be non-negative: " + probabilities[i]);
            sum += probabilities[i];
        }
        if (sum > 1.0 + EPSILON || sum < 1.0 - EPSILON)
            throw new IllegalArgumentException("sum of leetcode.array entries does not approximately equal 1.0: " + sum);
     ...
     * @throws IllegalArgumentException if all leetcode.array entries are {@code 0}
    ...
    public static int discrete(int[] frequencies) {
        if (frequencies == null) throw new IllegalArgumentException("argument leetcode.array must not be null");
        long sum = 0;
        for (int i = 0; i < frequencies.length; i++) {
            if (frequencies[i] < 0)
                throw new IllegalArgumentException("leetcode.array entry " + i + " must be non-negative: " + frequencies[i]);
            sum += frequencies[i];
        }
        if (sum == 0)
            throw new IllegalArgumentException("at least one leetcode.array entry must be positive");
    ...
    /**
     * Rearranges the elements of the specified leetcode.array in uniformly random order.
     *
     * @param  a the leetcode.array to shuffle
     ...
     * @return an leetcode.array of length {@code n} that is a uniformly random permutation
     ...
     * @return an leetcode.array of length {@code k} that is a uniformly random permutation
